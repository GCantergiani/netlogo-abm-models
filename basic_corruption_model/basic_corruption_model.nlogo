;; Ambiente
;; Si mis cercanos cometen un delito soy mas propenso a cometer un delito
;; Si arrestan a un cercano reduce mi posibilidad de cometer un delito // RATITO
;; Los politicos se mueven de su circulo
;; Los agentes se mueven entre los politicos

;; Cada agente:
;; ;; Tiene una aversión a la corrupcion
;; ;; Tiempo que ha estado en la carcel
;; ;; Posee un capital

;; Penalidades
;; Si cometo una falta soy encarcelado
;; Entre mas tiempo estoy encarcelado menos posibilidad existe de cometer una falta

;; Globales
;; ;; Gain: Porcentaje de ganancia (parametro global para ver si comenten un delito)
;; ;; Arrest: Posibilidad de que te arreste
;; ;; threshold: Umbral minino para ver si vale la pena o no cometer un delito
...
agents-own [
  ...
  jail-total
  financial-capital   ; initial capital
  forget-days
...
to setup
  ...
  ask patches [
    ...
    ;; Radio de vision para el vecindario
  ...
  ;; create agents
  ;; financial-capital sigue una distirbucion exponencial con tamaño maximo de 10
  create-agents round (initial-agent-density * .01 * count patches) [
    move-to one-of patches with [not any? turtles-here]
    ;;set heading 0
    set risk-aversion random-float 1.0 ;; 0 - 1
    set financial-capital (random-exponential 2 * 10) ;; 0 - 100
    set active? false
    set jail-term 0
    set jail-total 0
    set forget-days 0
...
to go
  ...
  ; Avanzo el tiempo de carcel
  ask agents
    [ if jail-term > 0 [ set jail-term jail-term - 1 ] 
      if forget-days > 0 [ set forget-days forget-days - 1 ] 
      if forget-days = 0 [ set jail-total 0 ] ]
...
to determine-behavior ;; risk-aversion que tanto se atreve a arriesgarse la persona
  set active? ( ( ( gain / financial-capital ) - (risk-aversion * estimated-arrest-probability) + environment - jail-total ) > threshold)
end

to-report gain ;; Problematicas menos la legitimidad del gobierno
  report (2 * bribe)
end

;; Probabilidad de ser arrestado
to-report estimated-arrest-probability ;; Probabilidad de que te arresten
  let C count (cops-on neighborhood) ;; Ve el número de policias que tiene el vecindario
  let Total count agents-on neighborhood
  report ( C / Total )
end
...
to-report environment
  ...
  let Ex 1 + count (agents-on neighborhood) with [jail-total > 0]
  let Total count agents-on neighborhood
  report ((A - Ex) / Total)
end

;; COP BEHAVIOR
to enforce ;; Si hay un agente alborotando
  if (random 100 < k) [
    if any? (agents-on neighborhood) with [active?] [
      ;; arrest suspect
      let suspect one-of (agents-on neighborhood) with [active?] ;; Busco uno y lo arresto
      ask suspect [ ;; arresto
        set active? false 
        let jail random max-jail-term
        set jail-term jail
        set jail-total (jail-total + ( jail / max-jail-term))
        set forget-days (max-jail-term * forget)
      ]
...
to display-agent-3D  ;; agent procedure
  set color scale-color green gain 1.5 -0.5
  ifelse active?
...
"" ""
PENS
"Normal" 1.0 0 -10899396 true "" "plot count agents with [not active? and jail-term = 0]"
"Carcel" 1.0 0 -16777216 true "" "plot count agents with [jail-term > 0]"
"Corrupto" 1.0 0 -2674135 true "" "plot count agents with [active?]"
